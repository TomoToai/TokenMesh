...
`ARK_API_KEY` is used for Ark model inference. `DEEPSEEK_API_KEY` is used for
DeepSeek Official API inference. The MVP model selector uses the static allowlist
in `app/src/lib/models.ts`, so deployment does not require Volcengine AK/SK or
Ark control-plane model discovery.
